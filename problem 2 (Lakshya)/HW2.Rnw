...
\begin{enumerate}
  ...
  Ensure to interpret each.
    ...
    var(X) = sigma^2
    skew(X) = 0
    The population skewness of the PDF is 0, which indicates that the Normal
    distribution is symmetric (with line of symmetry at the mean)
    kurt(X) = 0
  ...
  s1 %>% summarize(Mean = mean(x), SD = sd(x), Skewness = skewness(x), Kurtosis = kurtosis(x))
  @
  
  
  <<>>=
  s2 <- data.frame(x = rnorm(n = obs[2], mean = 0, sd = 1))
  s2 %>% summarize(Mean = mean(x), SD = sd(x), Skewness = skewness(x), Kurtosis = kurtosis(x))
  @
  
  <<>>=
  s3 <- data.frame(x = rnorm(n = obs[3], mean = 0, sd = 1))
  s3 %>% summarize(Mean = mean(x), SD = sd(x), Skewness = skewness(x), Kurtosis = kurtosis(x))
  @
  
  <<>>=
  s4 <- data.frame(x = rnorm(n = obs[4], mean = 0, sd = 1))
  s4 %>% summarize(Mean = mean(x), SD = sd(x), Skewness = skewness(x), Kurtosis = kurtosis(x))
  @
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%%%%%%%  Part (c)
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
